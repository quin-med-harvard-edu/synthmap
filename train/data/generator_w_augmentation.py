...
import tensorflow.keras.preprocessing.image as tfp
...
class DataGenerator_train(keras.utils.Sequence):
    ...
    def __init__(self, args, img_size, channel_size, input_img_paths, target_img_paths, noaugment=False):
        ...
        # over write augmentation options (for val and test options)
        if noaugment:
            self.augment_options['apply'] = False
    ...
    def __getitem__(self, idx):
        ...
        for j, (path_X,path_Y) in enumerate(zip(batch_input_img_paths,batch_target_img_paths)):
            ...
            if img_X.ndim == 4: 
                assert img_X.shape[-2] == 1, embed(header=sv.msg(f"Current shape is {img_X.shape}. Third dimension must be 1, not {img_X.shape[-2]}"))
                img_X = img_X[:,:,0,:]
            ...
            # if not img_Y.shape == self.img_size: 
            # 	# attempt to flip dimensions 
            # 	xx,yy = img_Y.shape
            # 	xx2,yy2 = self.img_size 
            # 	if yy2==xx and xx2 ==yy: 
            # 		img_Y.shape 
...
class DataGenerator_from_ids(keras.utils.Sequence):
    ...
    #def __init__(self, args, img_size, channel_size, input_img_paths, target_img_paths, noaugment=False):
    def __init__(self, args, img_size, channel_size, input_ids, noaugment=False):
        self.batchsize = args.batchsize
        self.img_size = img_size
        self.channel_size = channel_size
        self.normalize = args.normalize 
        ...
        self.xdir = args.xdir
        self.ydir = args.ydir
        self.xprefix = args.xprefix
        self.yprefix = args.yprefix
        self.xsuffix = args.xsuffix
        self.ysuffix = args.ysuffix
    ...
    def __len__(self):
        return self.input_ids.shape[0] // self.batchsize

    def __getitem__(self, idx):
        """Returns tuple (input, target) correspond to batch #idx."""
        i = idx * self.batchsize
        batch_ids = self.input_ids[i : i + self.batchsize, :]
        ...
        for j, ids in enumerate(batch_ids):
            ...
            path_X = self.xdir + "/" + self.xprefix + str(ids[0]) + "_s" + str(ids[1]) + self.xsuffix
            path_Y = self.ydir + "/" + self.yprefix + str(ids[0]) + "_s" + str(ids[1]) + self.ysuffix
...
class DataGenerator_from_ids_infinite(keras.utils.Sequence):
