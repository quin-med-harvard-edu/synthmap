...
def read_priors(opt):
    ...
    for p in ['vf_myelin', 'vf_ies', 'vf_csf']:
        ...
        if opt['std_to_mean_ratio'] is not None: 
            ...
            dfs_std[p] = dfs_mean[p].copy().multiply(opt['std_to_mean_ratio'][i]) # 10% 
        else: 
    ...
    opt['prior_means'], opt['prior_stds'],roi_map = convert_priors_to_synthseg_format_gauss_CSF(params_all_rois, opt)      
