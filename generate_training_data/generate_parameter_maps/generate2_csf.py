...
# input options 
import set_options,set_options_yaml_csf
...
def generate_from_single_label(datadir,opt):
    ...
    for n in range(opt['n_examples']):
        ...
        utils.save_volume(np.squeeze(lab), brain_generator.aff, brain_generator.header,
                    os.path.join(result_dir, 'params_labels_%s.nii.gz' % n), dtype=np.float32, nii_dtype=np.float32) #sv407 - saving labels as float32 (range 8d.pp)            
...
if __name__ == '__main__':
    ...
    if args.config is not None:
        ...
        opt,datadir = set_options_yaml_csf.prepare_opt(opt)    
    else:
